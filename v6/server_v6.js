 ...
 * 27-03-2018
 * Server.js_v6 - API no:04, according t Team-7 SRS.
...
app.post('/instructors', (req, res, next) => {                  //setStatusCode = 403 + 504
    ...
    Instructor.findOne({code : tempCacheCodeOfInput}, function(err, data) {
     ...
     //   console.log(data.code.length);
        //------------------------------
        else if( (tempCacheCodeOfInput.length == 2 || tempCacheCodeOfInput.length == 3 ) && ( data == null ) )
        {
        ...
        else if( tempCacheCodeOfInput.length > 3)
        {
            res.send("\tOops !!! Name code is not allowed more than 3 digits..");
        }


        else if( isLowerCase(req.body.code) )
        {
            res.send("Oops !!! LowerCase letters not allowed for name code.. try Capital letters..")
        }
        ...
        //     if( req.body.code.length == data.code.length)
        //     {
        //         res.send("Inside data != null, if condition, 555");
        //     }
        ...
        else if( data != null)
        {
            ...
            {
                res.send("\tdatabaseCreationError 507 " + data.code);
            }
        ...
        {
            res.send("\ttdatabaseCreationError 508 " + data );
        } 
...
app.delete('/instructors/:id', (req, res, next) => {
    ...
    if( req.params.id.length == 24 )
    {
        ...
        Instructor.findById( req.params.id, function(err, data) {
            ...
            } //End of 1st else if( (req.params.id.length) == 24;
        ...
        } //Part of outer Instructor.findOne( { code : req.params.id }, function(err, data);
        ) //End of outer Instructor.findOne( { code : req.params.id }, function(err, data), inside DEL[ ];
